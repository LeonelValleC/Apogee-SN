...
namespace Voyager_SN
{
    public partial class Reports : Form
    {
        ...
        private void EditExcel()
        {
            ...
            //Workbook workbook = excel.Workbooks.Open(@"C:\Users\leonel.valle\Desktop\Voyager SN.xlsx", ReadOnly: false, Editable: true);
        ...
        private void Search()
        {
            ...
            if (cb_Filter.Text == "Serial Number")
            {
                if (wo.Existe("select COUNT(*) from tb_Inprocess where SerialNumber = '" + txt_Search.Text + "'"))
                {
                    ...
                    salida_datos = "select inp.SerialNumber, inp.RegSN, pn.pn, pn.rev, wo.wo, usr.nemploy from tb_Inprocess inp join tb_WO wo on inp.id_wo = wo.id_wo join tb_PN pn on wo.id_pn = pn.id_pn join tb_User usr on  usr.id_user = inp.id_user where inp.id_inprocess = '" + inprocess.Id_inprocess + "' order by inp.id_inprocess asc";
                }
            ...
            if (cb_Filter.Text == "Work Order")
            {
                if (wo.Existe("select COUNT(*) from tb_WO where wo = '" + txt_Search.Text + "'"))
                {
                    ...
                    salida_datos = "select inp.SerialNumber, inp.RegSN, pn.pn, pn.rev, wo.wo, usr.nemploy, inp.Printed, inp.Validated, inp.Scrap from tb_Inprocess inp join tb_WO wo on inp.id_wo = wo.id_wo join tb_PN pn on wo.id_pn = pn.id_pn join tb_User usr on  usr.id_user = inp.id_user where wo.id_wo = '" + wo.Id_wo + "' order by inp.id_inprocess asc";
                }
            ...
            if (cb_Filter.Text == "Part Number")
            {
                if (wo.Existe("select COUNT(*) from tb_PN where pn = '" + txt_Search.Text + "'"))
                {
                    ...
                    salida_datos = "select inp.SerialNumber, inp.RegSN, pn.pn, pn.rev, wo.wo, usr.nemploy from tb_Inprocess inp join tb_WO wo on inp.id_wo = wo.id_wo join tb_PN pn on wo.id_pn = pn.id_pn join tb_User usr on  usr.id_user = inp.id_user where pn.id_pn = '" + pn.Id_pn + "' order by inp.id_inprocess asc";
                }
            ...
            if (cb_Filter.Text == "Date Register")
            {
                ...
                salida_datos = "select inp.SerialNumber, inp.RegSN, pn.pn, pn.rev, wo.wo, usr.nemploy from tb_Inprocess inp join tb_WO wo on inp.id_wo = wo.id_wo join tb_PN pn on wo.id_pn = pn.id_pn join tb_User usr on  usr.id_user = inp.id_user where CONVERT(varchar(255),inp.RegSN,126) LIKE '%" + buscar + "%' order by mo.id_wo asc";
            }
