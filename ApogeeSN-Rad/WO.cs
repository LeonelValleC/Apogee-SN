...
namespace Voyager_SN
{
    public partial class WO : Form
    {
        ...
        {
            cb_PN.DataSource = pn.LlenarComboBox("select id_pn, CONCAT(description,' | ',pcba) as 'pcb' from tb_PN");
            cb_PN.DisplayMember = "pcb";
        ...
        private void btn_Submit_Click(object sender, EventArgs e)
        {
            if (cb_PN.SelectedValue != null)
            {
                ...
                {
                    wo.Crud("insert into tb_WO(wo,qty,dateReg,id_pn, rev) values('" + txt_WO.Text + "','" + txt_QTY.Text + "','" + DateTime.Now + "','" + cb_PN.SelectedValue + "','" + txt_Rev.Text.Trim() + "')");
                    txt_QTY.Text = "";
                    txt_Rev.Text = "";
                    txt_WO.Text = "";
                    cb_PN.SelectedValue = -1;
        ...
        private void btn_Check_Click(object sender, EventArgs e)
        {
            ...
            cb_PN.Text = wo.ReturnFromROI("SELECT left(Part_Nbr,len(Part_Nbr)-3) FROM OPENQUERY([M2K81], 'select Part_Nbr from WO_v_1 where Wo_Nbr = ''" + txt_WO.Text + "''')");
        }
    }
}
